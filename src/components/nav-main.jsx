              ...
              items?.map((menu, index) => (
                menu?.hasSubMenu ?
                  <li key={index} className={`sidebar_menu overflow-hidden bg-white m-2 p-1 rounded-lg shadow uppercase  ${menu?.hasSubMenu ? "has_submenu" : ""}`}>
                    <a href='#' className='block p-2 rounded-lg border border-dashed pt-3 ps-5 relative overflow-hidden transition-all text-sm'>
                  ...
                  <li key={index} className='sidebar_menu bg-white m-2 p-1 rounded-lg shadow uppercase'>
                    <NavLink to={menu.url} className='block p-2 rounded-lg border border-dashed pt-3 ps-5 relative overflow-hidden transition-all text-sm'>
                      {menu.icon && React.createElement(menu.icon, { className: 'inline mb-1 me-1' })}
                      {menu.title}
                    </NavLink>
                  </li>
