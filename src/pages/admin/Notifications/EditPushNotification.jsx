...
export default function EditPushNotification() {
    ...
    const {
        ...
        queryFn: () => fetchNotificationById(token, notificationId),
        enabled: !!notificationId,
    });

    useEffect(() => {
        if (existingNotification?.data) {
            const notif = existingNotification.data;
            reset({
                title: notif.title,
                description: notif.description,
                redirection_type: notif.redirection_type,
                language: String(notif.language),
                datepick: notif.date,
                timepick: notif.time,
                ...
                states: notif.states?.map((s) => ({ label: s.name, value: s.id })),
    ...
    const updateMutation = useMutation({
        mutationFn: (data) => updateNotificationSchedule(notificationId, token, data),
        onSuccess: (res) => {
            if (res.success === 1) {
    ...
    return (
        <SidebarProvider>
            <AppSidebar />
            <SidebarInset>
                <AdminHeader head_text={notificationId ? "Edit Notification" : "Push Notification"} />
                    ...
                    >
                        <div className="form-heading bg-whitesmoke rounded-2xl mb-5 p-5 xl:col-span-4 col-span-1">
                            <h2 className="text-2xl font-bold text-center font-dmsans">
                                {notificationId ? "Edit Notification" : "Notification Schedule"}
                            </h2>
                        </div>
                        ...
                        <div className="form-submit-btn mt-5 xl:col-span-4 lg:col-span-3 col-span-1 rounded-2xl p-5 text-center bg-whitesmoke">
                            ...
                            >
                                {notificationId
                                    ...
                                    : addMutation.isPending ? "Scheduling..." : "Schedule Notification"}
                            </button>
                        </div>
                    </form>

                    {(addMutation.isPending || updateMutation.isPending || notificationLoading) && <Loader task="Processing..." />}
                </div>
            </SidebarInset>
        </SidebarProvider>
    );
